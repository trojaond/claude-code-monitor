 ...
 * @internal
 */
function isCcnHookCommand(command, eventName) {
 ...
 * @internal
 */
function hasCcnHookForEvent(entries, eventName) {
    if (!entries)
        return false;
 ...
 * @internal
 */
function hasGhosttySettingAsked(settings) {
 ...
 * @internal
 */
function applyGhosttyTitleSetting(settings, enabled) {
    if (!settings.env) {
        settings.env = {};
    }
 ...
 * @internal
 */
function createHookEntry(eventName, baseCommand) {
    const entry = {
        hooks: [
            {
                type: 'command',
 ...
 * @internal
 */
function categorizeHooks(settings) {
    const toAdd = [];
    const toSkip = [];
 ...
 */
export async function promptGhosttySettingIfNeeded() {
    const ghosttyInstalled = isGhosttyInstalled();
    if (!ghosttyInstalled)
        return;
    const settings = loadSettings();
    if (hasGhosttySettingAsked(settings))
        return;
    console.log('');
    console.log('Ghostty detected.');
    console.log('For reliable tab focus, Claude Code terminal title override should be disabled.');
