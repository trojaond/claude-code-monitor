...
export function sanitizeForAppleScript(str) {
    if (str.length > MAX_APPLESCRIPT_STRING_LENGTH) {
        throw new Error(`Input too long for AppleScript embedding (${str.length} > ${MAX_APPLESCRIPT_STRING_LENGTH})`);
    }
    const sanitized = str
 ...
 * @internal
 */
export function generateOscTitleSequence(title) {
 ...
 * @internal
 */
export function setTtyTitle(tty, title) {
...
function buildGhosttyFocusByTitleScript(titleTag) {
...
function raiseVSCodeWindow(workspaceName) {
...
export function focusSession(tty, cwd) {
    if (!isMacOS())
        return false;
    if (!isValidTtyPath(tty))
        return false;
